...
**Latency knobs (NVR-style mpegts, not LL-HLS):** hub `hlsSegmentDuration: 4s`,
`hlsSegmentCount: 10`, always-on RTSP pull; dashboard hls.js `liveSyncMode: buffered`,
`maxBufferLength: 30`, forward-only catch-up if >45s behind.
...
| Brief play then `hls_error` / RTP loss / DTS | Bitrate too high for WG remux, or HLS agent PATCHing MediaMTX every request. Set tower `CAMERA_RTSP_PATH` to **subtype=1** (quoted); deploy `hls_proxy.py` that never PATCHes; use buffered HLS (`hlsVariant: mpegts`), not lowLatency. |
