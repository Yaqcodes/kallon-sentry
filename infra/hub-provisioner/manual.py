...
try:  # works both as a package and as a flat script dir (hyphenated folder)
    from .interface import HubHost, HubProvider, resolve_ssh_identity, ssh_identity_args
except ImportError:  # pragma: no cover
    from interface import HubHost, HubProvider, resolve_ssh_identity, ssh_identity_args  # type: ignore
...
class ManualProvider(HubProvider):
    ...
    def provision(self, customer_id: str, *, host: str, ssh_user: str = "ubuntu",
        ...
        if not dry_run:
            try:
                identity_args = ssh_identity_args(require=True)
            except RuntimeError as exc:
                raise RuntimeError(str(exc)) from exc
            ident = resolve_ssh_identity()
            res = subprocess.run(
                ["ssh", "-o", "StrictHostKeyChecking=accept-new",
                 "-o", "ConnectTimeout=10", *identity_args,
                 f"{ssh_user}@{host}", "true"],
                capture_output=True, text=True, timeout=30,
            )
            if res.returncode != 0:
                hint = (
                    f" (identity={ident!r}; run scripts/kallon-hub-ssh-verify.ps1)"
                    if ident else ""
                )
                raise RuntimeError(
                    f"cannot SSH to {ssh_user}@{host}: {res.stderr.strip()}{hint}"
                )
        return hub
